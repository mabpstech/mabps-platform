import { headers } from "next/headers";
import {
  isWorkspaceManager,
  normalizeWorkspaceRole,
  type WorkspaceRole,
} from "@/lib/auth/permissions";
import { auth, type Session } from "@/lib/auth/server";
import { requireWorkspace } from "@/lib/auth/workspace";
import { ensureMemoryReady } from "@/lib/memory/repository";

export class MemoryAuthError extends Error {
  status: number;

  constructor(message: string, status = 403) {
    super(message);
    this.name = "MemoryAuthError";
    this.status = status;
  }
}

type MemoryWorkspace = {
  id: string;
  name: string;
  slug: string;
  logo?: string | null;
};

async function resolveMemoryContext(options: {
  managersOnly?: boolean;
}): Promise<{
  session: Session;
  workspace: MemoryWorkspace;
  role: WorkspaceRole;
}> {
  ensureMemoryReady();
  const requestHeaders = await headers();
  const session = await auth.api.getSession({ headers: requestHeaders });

  if (!session) {
    throw new MemoryAuthError("Authentication required.", 401);
  }

  const activeId = session.session.activeOrganizationId;
  if (!activeId) {
    throw new MemoryAuthError("No active workspace.", 400);
  }

  const fullOrg = await auth.api.getFullOrganization({
    headers: requestHeaders,
    query: { organizationId: activeId },
  });

  if (!fullOrg) {
    throw new MemoryAuthError("Workspace not found.", 404);
  }

  const membership = fullOrg.members?.find(
    (member) => member.userId === session.user.id,
  );
  if (!membership) {
    throw new MemoryAuthError("Not a workspace member.", 403);
  }

  const role = normalizeWorkspaceRole(membership.role);
  if (options.managersOnly && !isWorkspaceManager(role)) {
    throw new MemoryAuthError(
      "Only workspace owners and admins can perform this Memory Engine action.",
      403,
    );
  }

  return {
    session,
    workspace: {
      id: fullOrg.id,
      name: fullOrg.name,
      slug: fullOrg.slug,
      logo: fullOrg.logo,
    },
    role,
  };
}

/** Page-level: authenticated workspace member. */
export async function requireMemoryWorkspace(callbackUrl = "/memory") {
  const { session, workspace, role } = await requireWorkspace({
    callbackUrl,
  });
  ensureMemoryReady();
  return { session, workspace, role };
}

/** Any authenticated workspace member. */
export async function requireMemoryMemberApi() {
  return resolveMemoryContext({ managersOnly: false });
}

/** Owner/admin only. */
export async function requireMemoryManagerApi() {
  return resolveMemoryContext({ managersOnly: true });
}
